...
**文件路径**：
```
{session_dir}/{tool_id}_detailed.md
```
...
### 需要改进
{基于失分项生成，例如：}
- ⚠️ 可核查性有待提升（部分答案缺少独立引用）
- ⚠️ 效率可以优化（平均响应时间 5.2秒）
...
**文件路径**：
```
{session_dir}/summary_comparison.md
```
...
**报告位置**:
📁 {session_dir}/
  ├── summary_comparison.md (汇总对比)
  ├── {tool_1}_detailed.md
  ├── {tool_2}_detailed.md
  └── _test_log.jsonl (原始数据)

**快速结论**:
🥇 {tool_1}: {score}/120 ({pct}%)
🥈 {tool_2}: {score}/120 ({pct}%)
```
